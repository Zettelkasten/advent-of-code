...
all_lists = sorted([(i, j) for i in range(num_els) for j in range(num_els) if i < j and i not in clusters[j]], key=lambda x: dist(x[0], x[1]))

for closest_i, closest_j in all_lists[:1000]:
...
cluster_sizes = [len(cluster) for cluster in set(clusters.values())]
print("OUT", sorted(cluster_sizes))
print(numpy.prod(list(sorted(cluster_sizes))[-3:]))